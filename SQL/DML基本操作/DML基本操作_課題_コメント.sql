...
　⇒★結果はあってるけど以下のSQLが正解です。
　　　NEWEMPを"全件"集計してから'SALESMAN'を絞り込むのと、
　　　NEWEMPから'SALESMAN'を絞り込むんでから集計するのでは
　　　取り扱うデータの件数からどっちが早いかわかるかな？
...
　⇒★結果はあってるけどto_date/to_charについて月曜日お勉強しましょう。

回答例

/* 入社日の"年"の部分が1980であるもの */
UPDATE NEWEMP SET SAL = SAL + 200 WHERE TO_CHAR(HIREDATE, 'YYYY') = '1980';

/* 入社日の範囲が80/01/01～80/12/31のもの */
UPDATE NEWEMP SET SAL = SAL + 200 WHERE HIREDATE BETWEEN TO_DATE('19800101','YYYYMMDD') AND TO_DATE('19801231','YYYYMMDD');
